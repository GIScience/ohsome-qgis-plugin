...
from qgis.core import QgsMapLayerType, QgsMapLayer, QgsVectorLayer, QgsFeatureIterator, QgsFeature, QgsGeometry, \
    QgsWkbTypes, QgsPolygon, QgsMultiPolygon, QgsProject, Qgis, QgsField, QgsFields, QgsDataProvider, \
    QgsVectorDataProvider, QgsPoint, QgsUnitTypes
...
def create_time_aware_qgsfeature(qgsFeature: QgsFeature, startDateTime: QDateTime, endDateTime: QDateTime):
    pass
...
def load_layers_to_canvas(layer_list: list):
    ...
    for vlayer in layer_list:
        if not vlayer or not isinstance(vlayer, QgsVectorLayer) or not vlayer.isValid():
            iface.messageBar().pushMessage("[Utils] Error", "A layer couldn't be added.", level=Qgis.Warning)
        else:
...
def process_features(features: QgsFeatureIterator):
    ...
    for unprocessed_feature in features:
        ...
        key: QgsField
        for field, attribute in zip(unprocessed_feature.fields(), unprocessed_feature.attributes()):
            if isinstance(attribute, str) and (attribute.__contains__("node") or
                                               attribute.__contains__("way") or
                                               attribute.__contains__("relation")):
            ...
            elif isinstance(attribute, QDateTime) and field.name().__contains__("@snapshotTimestamp"):
        ...
        if new_fields.field('startDate') and new_fields.field('endDate'):
            ...
            if processed_features[feature_type].keys().__contains__(feature_id):
                ...
                processed_features[feature_type][feature_id].sort(key=lambda x: x.attribute("startDate"))
                last: QgsFeature = processed_features[feature_type][feature_id].__getitem__(
                    len(processed_features[feature_type][feature_id]) - 1)
                processed_features[feature_type][feature_id].__getitem__(
                    len(processed_features[feature_type][feature_id]) - 2).setAttribute(
...
def combine_features_to_layer(processed_features: dict, old_vlayer: QgsVectorLayer, activate_temporal: bool = True):
    ...
    for feature_type in processed_features:
        # create layer
        layer_type: str = ""
        if data_provider.wkbType() == QgsWkbTypes.Point or data_provider.wkbType() == QgsWkbTypes.PointGeometry:
            layer_type = "point"
        elif data_provider.wkbType() == QgsWkbTypes.LineString or data_provider.wkbType() == QgsWkbTypes.LineGeometry:
            layer_type = "linestring"
        elif data_provider.wkbType() == QgsWkbTypes.Polygon or data_provider.wkbType() == QgsWkbTypes.PolygonGeometry:
            layer_type = 'polygon'
            ...
            continue
        vlayer = QgsVectorLayer(layer_type,
        ...
        for feature_collection in processed_features[feature_type]:
            [features.append(x) for x in processed_features[feature_type][feature_collection]]
        if len(features) > 0:
...
def sort_and_combine_vector_layer(vlayer: QgsVectorLayer, activate_temporal: bool = True) -> list:
    processed_features = process_features(vlayer.getFeatures())
    if len(processed_features) <= 0:
        iface.messageBar().pushMessage("[Utils] Error", "Response empty.", level=Qgis.Warning)
        return list()
    return combine_features_to_layer(processed_features, vlayer, activate_temporal=activate_temporal)


def process_vector_layer(vlayer: QgsVectorLayer, activate_temporal: bool = True) -> list:
    processed_features = process_features(vlayer.getFeatures())
    if len(processed_features) <= 0:
        iface.messageBar().pushMessage("[Utils] Error", "Response empty.", level=Qgis.Warning)
        return list()

    return combine_features_to_layer(processed_features, vlayer.dataProvider(), activate_temporal=activate_temporal)
...
def get_layers():
    ...
    for layer_id, layer in layers.items():
        if layer.type() == QgsMapLayerType.VectorLayer and (
                layer.wkbType() == QgsWkbTypes.Polygon or layer.wkbType() == QgsWkbTypes.MultiPolygon):
...
def get_layer_by_name(name: str):
    if name:
        try:
            ...
            iface.messageBar().pushMessage(f"[Utils] Couldn't find layer {name}", f"{err}", level=Qgis.Warning)
    return None
