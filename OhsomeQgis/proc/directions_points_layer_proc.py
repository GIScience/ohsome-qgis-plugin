...
class ORSdirectionsPointsLayerAlgo(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for num, (points, from_value) in enumerate(
            zip(input_points, from_values)
        ):
    ...
    def _get_params_directions(points, profile, preference):
        ...
        params = {
            "coordinates": [
                [round(point.x(), 6), round(point.y(), 6)] for point in points
            ],
    ...
    def _get_params_optimize(points, profile):
        ...
        for point in points:
            params["jobs"].append(
                {"location": [point.x(), point.y()], "id": points.index(point)}
            )
