...
from OhsomeQgis.utils import configmanager, transform, exceptions, logger
...
class ORSdirectionsPointsLayersAlgo(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(
        self, configuration, p_str=None, Any=None, *args, **kwargs
    ):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        preference = PREFERENCES[
            self.parameterAsEnum(parameters, self.IN_PREFERENCE, context)
        ]

        mode = self.MODE_SELECTION[
            self.parameterAsEnum(parameters, self.IN_MODE, context)
        ]

        # Get parameter values
        source = self.parameterAsSource(parameters, self.IN_START, context)
        source_field_name = self.parameterAsString(
            parameters, self.IN_START_FIELD, context
        )
        ...
        route_dict = self._get_route_dict(
            source, source_field, destination, destination_field
        )

        if mode == "Row-by-Row":
            route_count = min(
                [source.featureCount(), destination.featureCount()]
            )
        else:
    ...
    def _get_route_dict(
        self, source, source_field, destination, destination_field
    ):
        """
